...
Een eerste vereenvoudiging zal het probleem van een kwadratisch aantal pinnen (_n_<sup>2</sup> voor een _n_×_n_-bord) reduceren naar een lineair aantal (2_n_). Hiervoor verbinden we alle knoppen in een rij van het rooster met eenzelfde outputpin en alle knoppen in een kolom van het rooster met eenzelfde inputpin, zoals op de figuur.

![9buttons](../assets/images/project/9buttons.png "negen knoppen")

De Arduino zal herhaaldelijk elk van de inputpinnen eens op `LOW` zetten, voor elk van de outputpinnen controleren of die een `LOW` signaal opvangt, en de inputpin dan weer terugzetten op `HIGH`. Uit de informatie welke outputpin `LOW` geeft bij welke inputpin valt zo te achterhalen welke drukknop precies wordt ingedrukt. Onderstaand programma geeft het principe weer. Dit keer laten we niet gewoon een LEDje oplichten maar schrijft de Arduino naar de console uit welke drukknop gedetecteerd wordt.

```c++
const int ROWS[3] = {7, 6, 5}; //top to bottom
const int COLS[3] = {2, 3, 4}; //left to right
