...
![shiftregister1](../assets/images/project/shiftregister.png "schuifregister")
 ...
 - *power* vereist een constante voeding van `HIGH` en wordt dus best aangesloten op de 5V-uitgang van de Arduino;
 - *ground* vereist een constante voeding van `LOW` en wordt dus best aangesloten op de grond van de Arduino;
 ...
 - *output 8\** geeft een kopie van output 8 die toelaat aan een volgend schuifregister door te koppelen.

We hebben in Tinkercad een klein project gebouwd om de mogelijkheden te demonstreren. Om nog wat elektronica te leren, gebruiken we dit keer een 7 segment LED display als visuele feedback. Deze sketch functioneert als een heel rudimentaire klok die seconden telt. De Arduino stuurt via outputpin 3 telkens een byte door, die bepaalt welke segmenten van de display oplichten.

![shiftregister2](../assets/images/project/shiftregister2.png "schuifregister")

De functie `shiftOut()` stuurt telkens de data bit per bit door en activeert de klok van het schuifregister, waarna de latch wordt geactiveerd en juiste LEDs oplichten.
