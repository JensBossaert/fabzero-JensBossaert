...
Het voornaamste doel van multiplexing is meer data langs eenzelfde kanaal te kunnen zenden (ten koste van tijd): initieel worden de vele datalijnen gemuxt naar een enkel kanaal, dat de data één per één kan doorsturen, waarna op het einde de data weer kan worden gedemuxt over meerdere datalijnen. Voor onze toepassing willen we het omgekeerde doen: een enkel datasignaal (namelijk een constante `HIGH`) verdelen over herhaaldelijk een volgende rij van het rooster, en dan telkens de acht signalen uit de kolommen oppikken naar een enkel inputsignaal voor de Arduino.

Concreet gebruiken we eerst een demultiplexer, zoals de 74HC237.
...
Het gebruik van multiplexing betekent dat we in totaal één pin meer nodig zullen hebben. De schuifregisters konden met een constant aantal pinnen functioneren, namelijk zes in totaal voor een arbitraire _n_×_n_-matrix, maar de multiplexers gebruiken elk ⌈log<sub>2</sub>(_n_)⌉ selectorlijnen en een extra datalijn. Merk op dat we de input van de demultiplexer in onze situatie constant op `HIGH` kunnen houden, dus dat spaart één pin uit. In totaal hebben we dan 3 + 3 + 1 pinnen nodig.

Waarom kiezen we dan niet toch voor de schuifregisters? Wel, conceptueel zijn de multiplexers eenvoudiger, meer op maat gemaakt dan de schuifregisters, en ook een fractie sneller (in microseconden, dus absoluut irrelevant voor onze doeleinden). Daarnaast lijken (de-) multiplexers ietwat makkelijker en goedkoper te vinden dan een PISO-schuifregister. Die ene pin lijkt een redelijke afweging voor een winst in eenvoud in het ontwerpproces.

We kunnen het nog wat verbeteren door gebruik te maken van de latchmogelijkheid van de demultiplexer: dan hebben we één pin nodig de latch aanstuurt, één pin die de data inleest, en kunnen we drie selectorlijnen gebruiken om zowel de multiplexer als de demultiplexer aan te sturen. Vijf pinnen volstaan dus!
